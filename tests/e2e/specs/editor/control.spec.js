...
test.describe( 'Document setting control', () => {
	test( `checking checkbox enables feature and creates link`, async ( {
		page,
		admin,
		editor,
	} ) => {
		...
		await expect(
			page.getByRole( 'textbox', { name: 'Secret URL:' } )
		).toBeVisible();
		...
		expect( meta[ '_sppp_key' ] ).toBe(
			linkUrl.searchParams.get( '_sppp_key' )
		);
	} );

	test( `disabling checkbox removes key and disables link`, async ( {
		page,
		admin,
		editor,
	} ) => {
