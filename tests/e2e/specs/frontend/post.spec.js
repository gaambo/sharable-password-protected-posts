...
async function logout(page) {
    const logoutLink = await page.locator( '#wp-admin-bar-logout > a' ).getAttribute("href");
    return page.goto(logoutLink);
}

test.describe("Frontend viewing post", () => {
    test(`can view private post by visiting secret URL`,  async ({page, admin , editor, requestUtils}) => {
    ...
    test(`cannot view private post being logged out`,  async ({page, admin , editor, requestUtils}) => {
        await admin.createNewPost();
    ...
    test(`can view private post by being logged in`,  async ({page, admin , editor}) => {
        await admin.createNewPost();
